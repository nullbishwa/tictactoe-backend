...
const LUDO_CONFIG = {
    ...
    MAX_STEPS: 57 // 51 path + 6 home stretch
};
        ...
        try {
            ...
            if (msg.type === 'QUIT_REQUEST') {
                ...
                room.clients.forEach((role, client) => {
                    if (role !== myRole && client.readyState === 1) client.send(res);
                });
                return;
            }

            if (msg.type === 'QUIT_RESPONSE') {
                if (msg.accepted) {
                    const res = JSON.stringify({ type: 'GAME_ENDED', reason: 'Mutual Agreement' });
                    room.clients.forEach((role, client) => { if (client.readyState === 1) client.send(res); });
                    rooms.delete(roomId);
                } else {
                    const res = JSON.stringify({ type: 'QUIT_REJECTED' });
                    room.clients.forEach((role, client) => {
                        if (role !== myRole && client.readyState === 1) client.send(res);
                    });
                }
                ...
                if (isMoveLegal(msg.from, msg.to, room.board, myColor, room)) {
                    ...
                    if (!nextMoves) {
                        const finalState = JSON.stringify({
                            ...
                            // winReason triggers your specialized "Loved Link" UI
                            winReason: inCheck ? "CHECKMATE" : "STALEMATE",
                            isDraw: !inCheck,
                            hubbyColor: room.roles.Hubby,
                            wiifuColor: room.roles.Wiifu
                        });
